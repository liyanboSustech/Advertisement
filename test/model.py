...
class BaselineModel(torch.nn.Module):
    ...
    def __init__(self, user_num, item_num, feat_statistics, feat_types, args):  #
        ...
        self.temperature = 0.7  # InfoNCE temperature parameter
    ...
    def feat2emb_from_tensors(self, seq, feature_tensors, mask=None, include_user=False):
        """
        Args:
            seq: 序列ID
        ...
        # batch-process each feature type using pre-processed tensors
        for feat_dict, feat_type, feat_list in all_feat_types:
            if not feat_dict:
                continue
        ...
        # Handle item embedding features
        for k in self.ITEM_EMB_FEAT:
            if k in feature_tensors:
                tensor_feature = feature_tensors[k].to(self.dev)
                item_feat_list.append(self.emb_transform[k](tensor_feature))
    ...
    def log2feats(self, log_seqs, mask, seq_feature):
        ...
        Args:
            ...
            seq_feature: 序列特征，可以是list形式或tensor字典形式
        ...
        if isinstance(seq_feature, dict):
            seqs = self.feat2emb_from_tensors(log_seqs, seq_feature, mask=mask, include_user=True)
        else:
    ...
    def forward(
        ...
        """
        训练时调用，计算正负样本的embeddings用于InfoNCE loss
        ...
        Args:
            ...
            seq_feature: 序列特征，可以是list形式或tensor字典形式
            pos_feature: 正样本特征，可以是list形式或tensor字典形式
            neg_feature: 负样本特征，可以是list形式或tensor字典形式

        Returns:
            seq_embs: 序列embeddings，形状为 [batch_size, maxlen, hidden_dim]
            pos_embs: 正样本embeddings，形状为 [batch_size, maxlen, hidden_dim]
            neg_embs: 负样本embeddings，形状为 [batch_size, maxlen, hidden_dim]
        ...
        # 检查是否为预处理的tensor格式
        if isinstance(pos_feature, dict) and isinstance(neg_feature, dict):
            # 使用预处理的tensor格式
            pos_embs = self.feat2emb_from_tensors(pos_seqs, pos_feature, include_user=False)
            neg_embs = self.feat2emb_from_tensors(neg_seqs, neg_feature, include_user=False)
        else:
            # 使用原始特征格式
    ...
    def predict(self, log_seqs, seq_feature, mask):
        ...
        # Apply normalization for consistency with training
