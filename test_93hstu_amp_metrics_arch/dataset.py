...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, args, dataset_type='train'):
        ...
        if dataset_type in ['train', 'valid']:
            self._split_train_valid(args.valid_ratio if hasattr(args, 'valid_ratio') else 0.1)
    ...
    def _split_train_valid(self, valid_ratio):
        """划分训练集和验证集"""
        all_uids = list(self.seq_offsets.keys())
        train_uids, valid_uids = train_test_split(
            all_uids, test_size=valid_ratio, random_state=42
        )
        
        if self.dataset_type == 'train':
            self.seq_offsets = {uid: self.seq_offsets[uid] for uid in train_uids}
        elif self.dataset_type == 'valid':
            self.seq_offsets = {uid: self.seq_offsets[uid] for uid in valid_uids}
