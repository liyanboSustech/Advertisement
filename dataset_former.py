...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, args):
        ...
        self._init_feat_tensor_info()
        
    def _init_feat_tensor_info(self):
        self.feat_max_len = {}
    
        for k in self.feature_types['item_array'] + self.feature_types['user_array']:
            self.feat_max_len[k] = 0 
    
    def feat2tensor(self, feat, k):
        """将单个特征转换为tensor"""
        if k in self.feature_types['item_array'] or k in self.feature_types['user_array']:
            # 数组类型特征处理
            max_len = self.feat_max_len.get(k, 0)
            if max_len == 0:
                max_len = len(feat)  # 使用实际长度
            
            # 初始化并填充数据
            tensor_data = np.zeros(max_len, dtype=np.int64)
            actual_len = min(len(feat), max_len)
            tensor_data[:actual_len] = feat[:actual_len]
    ...
    def process_features(self, feat_dict):
        """处理单条记录的所有特征，转换为tensor"""
        processed_feat = {}
        for k, v in feat_dict.items():
            processed_feat[k] = self.feat2tensor(v, k)
