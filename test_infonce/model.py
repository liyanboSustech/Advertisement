...
class BaselineModel(torch.nn.Module):
    ...
    def feat2emb(self, seq, feature_array, mask=None, include_user=False):
        ...
        Args:
            seq: 序列ID
            feature_array: 预处理好的特征字典，每个特征ID对应一个tensor
    ...
    def forward(
        ...
        """
        # log_feats的作用是什么？
        # log_feats是用户序列的特征表示，用于计算正负样本的相似度
        log_feats = self.log2feats(user_item, mask, seq_feature)
        # lossmask的作用是什么
        # lossmask用于指示哪些位置是有效的，只有有效位置的损失才会被计算
        ...
        # pos_logits是什么意思
        # pos_logits是用户序列与正样本之间的相似度得分
        # *代表什么乘法？
        # 这里的乘法是元素级别的乘法，表示用户序列和正样本之间的相似度
        pos_logits = (log_feats * pos_embs).sum(dim=-1)
        # 为什么要把最后一维求和？
        # 因为我们需要得到每个样本的最终相似度得分，所以对特征维度进行求和
        # 他原本的维度和乘法之后的维度以及sum之后的维度分别是怎么样的？
        #   原始维度: [batch_size, hidden_units]
        #   乘法之后的维度: [batch_size, hidden_units]
        #   sum之后的维度: [batch_size]
        neg_logits = (log_feats * neg_embs).sum(dim=-1)
        pos_logits = pos_logits * loss_mask
        neg_logits = neg_logits * loss_mask

        return pos_logits, neg_logits, seq_embs, pos_embs, neg_embs, loss_mask
        ...
        for start_idx in tqdm(range(0, len(item_ids), batch_size), desc="Saving item embeddings"):
            ...
            for feat_id in self.ITEM_SPARSE_FEAT.keys():
                ...
                for i in range(start_idx, end_idx):
                    if feat_id in feat_dict[i]:
                        feat_list.append([feat_dict[i][feat_id]])
                    else:
                        feat_list.append([0])
                batch_feat[feat_id] = torch.tensor(feat_list, device=self.dev)
            ...
            for feat_id in self.ITEM_EMB_FEAT.keys():
                feat_list = []
                for i in range(start_idx, end_idx):
                    if feat_id in feat_dict[i]:
                        feat_list.append([feat_dict[i][feat_id]])
                    else:
                        feat_list.append([np.zeros(self.ITEM_EMB_FEAT[feat_id], dtype=np.float32)])
                batch_feat[feat_id] = torch.tensor(feat_list, device=self.dev)
