...
class BaselineModel(nn.Module):
    ...
    def compute_infonce_loss(self, seq_embs, pos_embs, neg_embs,pos_seqs,neg_seqs, loss_mask, writer):
        ...
        # 1. 正样本相似度：每个样本与自己的正样本
        pos_logits = (seq_embs_normalized * pos_embs_normalized).sum(dim=-1).unsqueeze(-1)  # [B, L, 1]
        writer.add_scalar("Model/nce_pos_logits", pos_logits.mean().item())

        # 2. 负样本相似度：与所有 in-batch 负样本
        B, L, _ = seq_embs.shape
        neg_logits = torch.matmul(seq_embs_normalized, neg_embs_normalized.transpose(-1, -2))  # [B, L, N_neg]
        writer.add_scalar("Model/nce_neg_logits", neg_logits.mean().item())

        # 3. 构造 false negative mask
        # pos_seqs 需要传进来；这里假设你额外传入了 pos_seqs: [B, L]
        # 如果 pos_seqs 没传，可在 forward() 里一并保存下来
        mask = (neg_seqs.unsqueeze(1) == pos_seqs.unsqueeze(2)).any(1)  # [B, L, N_neg]
        neg_logits = neg_logits.masked_fill(mask, -1e9)

        # 4. 合并 logits 并计算 loss（与原逻辑一致）
        logits = torch.cat([pos_logits, neg_logits], dim=-1)  # [B, L, 1 + N_neg]
