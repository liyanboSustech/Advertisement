...
def get_candidate_emb(indexer, feat_types, feat_default_value, mm_emb_dict, model):
    ...
    # 保存候选库的embedding和id
    # 注意：对于候选库，我们不传递时间戳，因为这是静态的item embedding
    model.save_item_emb(item_ids, retrieval_ids, features, os.environ.get('EVAL_RESULT_PATH'), timestamps=None)
...
def infer():
    ...
    for step, batch in tqdm(enumerate(test_loader), total=len(test_loader)):
        ...
        # 调用predict方法，传入时间戳
        logits = model.predict(seq, seq_feat, token_type, timestamps=timestamps)
